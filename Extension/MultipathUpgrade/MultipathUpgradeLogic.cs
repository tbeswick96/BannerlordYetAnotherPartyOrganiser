...
namespace YAPO.MultipathUpgrade {
    public static class MultipathUpgradeLogic {
        ...
        public static bool TryGetUpgradePaths(PartyCharacterVM troops, out List<(int, PartyScreenLogic.PartyCommand.UpgradeTargetType)> upgradeTargets) {
            int upgradeIndexByBandit = GetUpgradePathIndexByIsBandit(troops);

            if (upgradeIndexByBandit > -1) {
                upgradeTargets = new List<(int, PartyScreenLogic.PartyCommand.UpgradeTargetType)> {(troops.NumOfUpgradeableTroops, (PartyScreenLogic.PartyCommand.UpgradeTargetType) upgradeIndexByBandit)};
                return true;
            }
            ...
            if (upgradeIndexByCultureStrength > -1) {
                upgradeTargets = new List<(int, PartyScreenLogic.PartyCommand.UpgradeTargetType)> {(troops.NumOfUpgradeableTroops, (PartyScreenLogic.PartyCommand.UpgradeTargetType) upgradeIndexByCultureStrength)};
                return true;
            }
        ...
        private static IEnumerable<(int, PartyScreenLogic.PartyCommand.UpgradeTargetType)> DetermineMaxCountOfUpgradesPerTarget(PartyCharacterVM troops, IList<PartyScreenLogic.PartyCommand.UpgradeTargetType> upgradeTargets) {
        ...
        private static int GetUpgradePathByCultureStrength(PartyCharacterVM troops) {
            List<UpgradeCandidate> candidates = troops.Character.UpgradeTargets.Select((x, index) => new UpgradeCandidate {
                                                           ...
                                                           UpgradeClassTipsWhichAreSpecialties = UpgradeTreeCrawler.GetUpgradeTreeTips(x)
                                                                                                                   .Where(y => GetPreferredClassTypesByCulture(troops.Character.Culture.ToString()).Any(z => z == y.ClassType))
                                                                                                                   .ToList()
                                                       })
                                                      .ToList();
            ...
            List<CharacterClassType> firstNotSecond = candidates.First().UpgradeClassTipsWhichAreSpecialties.Select(c => c.ClassType).Except(candidates.Last().UpgradeClassTipsWhichAreSpecialties.Select(c => c.ClassType)).ToList();
            List<CharacterClassType> secondNotFirst = candidates.Last().UpgradeClassTipsWhichAreSpecialties.Select(c => c.ClassType).Except(candidates.First().UpgradeClassTipsWhichAreSpecialties.Select(c => c.ClassType)).ToList();
        ...
        private static UpgradeCandidate GetCandidateWithWeaponType(IEnumerable<UpgradeCandidate> candidates, EquipmentProperties equipmentProperty) {
            List<UpgradeCandidate> superCandidates = candidates.SelectMany(x => x.UpgradeClassTipsWhichAreSpecialties, (candidate, upgradeCharacter) => new {candidate, upgradeCharacter})
                                                               ...
                                                               .Where(x => x.candidate.UpgradeClassTipsWhichAreSpecialties.Any(y => (y.EquipmentProperties & equipmentProperty) != 0))
