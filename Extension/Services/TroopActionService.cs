...
namespace YAPO.Services {
    public static class TroopActionService {
        ...
        private static List<Tuple<PartyCharacterVM, PartyScreenLogic.PartyCommand>> PrepareUpgradeCommands(IEnumerable<PartyCharacterVM> upgradableTroops, PartyScreenLogic partyScreenLogic, ref UpgradeResults upgradeResults) {
            ...
            foreach (PartyCharacterVM troops in upgradableTroops) {
                List<(int, PartyScreenLogic.PartyCommand.UpgradeTargetType)> upgradesPerTypes =
                    new List<(int maxUpgradableTroops, PartyScreenLogic.PartyCommand.UpgradeTargetType upgradeTargetType)> {(int.MaxValue, PartyScreenLogic.PartyCommand.UpgradeTargetType.UpgradeTarget1)};
        ...
        private static List<PartyCharacterVM> GetRecruitablePrisoners(PartyVM partyVm) {
            return partyVm.MainPartyPrisoners.Where(x => !x.IsHero && x.IsRecruitablePrisoner && x.NumOfRecruitablePrisoners > 0).ToList();
        }

        private static List<PartyCharacterVM> GetUpgradeableTroops(PartyVM partyVm) {
            return partyVm.MainPartyTroops.Where(x => !x.IsHero && x.IsTroopUpgradable && x.IsUpgrade1Available && !x.IsUpgrade1Insufficient && x.NumOfTarget1UpgradesAvailable > 0 ||
                                                      x.IsUpgrade2Available && !x.IsUpgrade2Insufficient && x.NumOfTarget2UpgradesAvailable > 0)
                          .ToList();
        }
        ...
        private static int CalculateCountOfTroopsThatCanBeUpgraded(AvailableResources availableResources,
            ...
            if (upgradeRequiredItemCategory != null && !availableResources.ItemsOfCategoryWithCount.ContainsKey(upgradeRequiredItemCategory)) {
                availableResources.ItemsOfCategoryWithCount.Add(upgradeRequiredItemCategory, troops.GetNumOfCategoryItemPartyHas(partyScreenLogic.RightOwnerParty.ItemRoster, upgradeRequiredItemCategory));
            }

            int upgradeCost = troops.Character.UpgradeCost(PartyBase.MainParty, 0) <= 0 ? 1 : troops.Character.UpgradeCost(PartyBase.MainParty, (int) upgradesPerTargetType.upgradeTargetType);
        ...
        private static void ExecuteCommands(IEnumerable<Tuple<PartyCharacterVM, PartyScreenLogic.PartyCommand>> commands, PartyVM partyVm, PartyScreenLogic partyScreenLogic) {
